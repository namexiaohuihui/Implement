...
public class MutualJsonBean<T> {
    ...
    public GoodsBean jsonToBean(String resJson) {
        //一般情况下，我们从服务器返回来的数据是json数据，所以我们需要把它转换成bean对应的实体类
        //这里有一点要注意的是，我们进行一般的转换，需要json和bean的属性数量和属性类型和名称都要对应完全一致
        //否则可能会报错
        Gson gson = new Gson();
        GoodsBean gb = gson.fromJson(resJson, GoodsBean.class);//将json数据转换成user类实体
        return gb;
    }
    ...
    public String beanTojson() {
        ...
        String strUser1 = gson.toJson(gb);//将bean数据转换成json数据
        return strUser1;
    }

    public GoodsBean limitJsonToBean(String resJson) {
        ...
        GoodsBean gb = gs.fromJson(resJson, GoodsBean.class);
        return gb;
    }
